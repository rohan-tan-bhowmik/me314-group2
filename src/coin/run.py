...
class run(Node):
    ...
    def stage_machine(self):
        ...
        if self.stage == 0:
            ...
            if self.coin_position == None or self.goal_position == None:
                ...
                self.get_logger().info("Both coin and green square found")
                self.stage += 1
            ...
            self.get_logger().info("Releasing coin")
            self.stage += 1
    ...
    def coin_pos_callback(self, point: Point):
        if (self.coin_initialized):
            return
        self.coin_position = point
        if (point != None):
